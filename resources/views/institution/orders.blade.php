...
<div class="m-10">
    @foreach ($groupedOrders as $product)
        <details class="mb-8 p-6 bg-gray-100 rounded-lg">
            <summary class="cursor-pointer text-lg font-bold text-[#212B36] focus:outline-none">
                {{ $product['product'] }}
            </summary>
            ...
            <div class="mt-4">
                ...
                @foreach (($product['parts'] ?? []) as $partKey => $part)
                    <details class="mt-4 pl-4">
                        <summary class="cursor-pointer text-md font-semibold text-[#212B36] focus:outline-none">
                            {{ $part['part'] ?? $partKey }}
                            {{-- Si decidiste guardar colors para mostrar, puedes descomentar esto:
                            @if(!empty($part['colors']))
                                <span class="text-sm font-normal text-gray-600">({{ implode(', ', $part['colors']) }})</span>
                            @endif
                            --}}
                        </summary>
                        ...
                        <div class="mt-2">
                            <table class="font-inter w-full table-auto border-separate border-spacing-y-1 overflow-scroll text-left md:overflow-auto pl-4">
                                <thead class="w-full rounded-lg bg-[#222E3A]/[6%] text-base font-semibold text-white">
                                    <tr>
                                        ...
                                        @foreach ($availableSizes as $size)
                                            <th class="whitespace-nowrap py-3 text-sm font-normal text-[#212B36] text-center" width="5%">
                                                {{ $size }}
                                            </th>
                                        @endforeach
                                    </tr>
                                ...
                                <tbody>
                                    {{-- ✅ Orders ahora viven en $part['orders'] --}}
                                    @foreach (($part['orders'] ?? []) as $clientId => $order)
                                        <tr>
                                            <td class="whitespace-nowrap py-3 pl-3 text-sm font-normal text-[#212B36]">
                                                {{ $order['client'] }}
                                            </td>
                                            <td class="py-3 text-sm font-normal text-[#212B36]" style="width: 150px; word-wrap: break-word;">
                                                {{ $order['embroidery'] ?? 'N/A' }}
                                            </td>
                                            <td class="py-3 text-sm font-normal text-[#212B36]" style="width: 150px; word-wrap: break-word;">
                                                {{ $order['special_size'] ?? 'N/A' }}
                                            </td>
                                    ...
                                    {{-- ✅ Totales ahora viven en $part['totals'] --}}
